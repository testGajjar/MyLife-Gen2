...
class UserImage(ndb.Model):
	...
	def import_image(self, filename, original_filename, image_data, date, email_attachment_content_id):

		content_type = self.get_content_type(original_filename)
		print('Inside import_image')
		self.original_size_key = filename
		filestore.write(self.original_size_key, image_data, content_type)
		...
		if image.width <= MAX_SIZE and image.width <= MAX_SIZE:
			...
			resized_bytes = images.resize(image_data, MAX_SIZE, MAX_SIZE)
